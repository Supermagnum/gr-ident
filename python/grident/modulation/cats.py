"""Raw 2-FSK 9600 baud modulator/demodulator for CATS preamble air interface."""
...
# Baseband mark/space for gr-ident test vectors (4800 Hz total shift).
CATS_MARK_HZ = -2400.0
CATS_SPACE_HZ = 2400.0


def samples_per_symbol(sample_rate: int = DEFAULT_SAMPLE_RATE) -> int:
    if sample_rate % CATS_BAUD:
        raise ValueError("sample_rate must be an integer multiple of 9600")
    return sample_rate // CATS_BAUD


def bit_frequency(bit: int) -> float:
    return CATS_MARK_HZ if int(bit) & 1 else CATS_SPACE_HZ
...
def demodulate_fsk2(
    ...
    offset = start
    count = 0
    freqs = (CATS_SPACE_HZ, CATS_MARK_HZ)
